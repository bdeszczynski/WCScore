...
- `admin.html` links to the manual GitHub Actions and includes a result override builder.
...
- `public/data/manual-results.json` is the manual result override file generated by the admin builder and applied after API refreshes.
...
- Manual corrections: the admin page can prepare `public/data/manual-results.json`; those overrides win over API results by match ID after each refresh until removed.
